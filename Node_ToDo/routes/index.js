...
router.post("/", function (req, res) {
  ...
  // 이 데이터를 통째로 구조까지 똑같이 todo_tada로 복제하라
  let todo_data = { ...req.body };

  // res.send(todo);
  // res.json(req.body);
  // JSON.strinify(isonObject) : jsonObject(JSON객체) 데이터를
  // 네트워크를 통해서 전송하기위하여 문자열로 변환시키기
...
// post("/insert") : form에서 입력 받은 내용을 서버로 전송했을떄
