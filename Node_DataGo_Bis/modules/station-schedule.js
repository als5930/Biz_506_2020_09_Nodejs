...
const execStation = () => {
  ...
  // requset를 통해서 reqOption에 담긴 방식으로
  ...
  request(reqOption, async (err, response, body) => {
    ...
    console.log("노선정보 데이터 추가 개수 :", station_list.length);
...
export const bis_Schedule = () => {
  // cron type 스케쥴러
  // 초 분 시 일 월 요일 단위로 스케쥴링을 수행하는코드
  // 알정한 시간에 execStation 함수를 실행하여
  // openAPI로 부터 데이터를 가져와서 DB에 insert 수행할 것이다
  // 매주 일요일(0) 12시 1분 8초에 실행하라
